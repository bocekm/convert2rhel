...
def replace_non_red_hat_packages():
    ...
    call_yum_cmd_w_downgrades("update", append_subscription_manager(orig_os_pkgs))

    loggerinst.info(
        "Performing reinstallation of the %s packages ..." % system_info.name)
    orig_os_pkgs = get_installed_pkgs_by_fingerprint(system_info.fingerprints_orig_os)
    call_yum_cmd_w_downgrades("reinstall", append_subscription_manager(orig_os_pkgs))
    ...
    orig_os_pkgs = get_installed_pkgs_by_fingerprint(system_info.fingerprints_orig_os)
    call_yum_cmd_w_downgrades(cmd, append_subscription_manager(orig_os_pkgs))


def append_subscription_manager(pkgs_to_replace):
    ...
    if system_info.id == "oracle" and system_info.version.major == 6:    
        return pkgs_to_replace.append("subscription-manager*")
    else:
        return pkgs_to_replace
